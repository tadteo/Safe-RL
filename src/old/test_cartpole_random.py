...
###############################################################################
    ...
    def train_model(self):
        ...
        # print("Q_val: ",type(target), target.shape, target)
...
###############################################################################
        ...
        for i in range(self.batch_size): #For every batch element
            ...
            if done[i] == False:
                target[i][action[i]] += self.discount_factor * np.amax(target_val[i])
...
###############################################################################
    ...
    def plot_data(self, episodes, scores, max_q_mean):
        ...
        pylab.figure(0)
        pylab.plot(episodes, max_q_mean, 'b',linestyle="",marker="o")
        pylab.xlabel("Episodes")
        pylab.ylabel("Average Q Value")
        pylab.savefig("normal_{}qvalues_{}.png".format(dt_string,self.id_nr))

        pylab.figure(1)
        pylab.plot(episodes, scores, 'b')
        pylab.xlabel("Episodes")
        pylab.ylabel("Score")
        pylab.savefig("normal_{}scores_{}.png".format(dt_string,self.id_nr))
            ...
            if done:
                ...
                if agent.check_solve:
                    if np.mean(scores[-min(100, len(scores)):]) >= 195:
                        ...
                        sys.exit()
    plot.plot_data(agent,episodes,scores,max_q_mean,"random")
